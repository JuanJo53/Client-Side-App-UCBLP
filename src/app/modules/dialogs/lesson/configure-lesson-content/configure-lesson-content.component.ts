...
@Component({
  selector: "app-configure-lesson-content",
  templateUrl: "./configure-lesson-content.component.html",
  styleUrls: ["./configure-lesson-content.component.scss"],
})
export class ConfigureLessonContentComponent implements OnInit {
  nombre: string = "";
  numero: number = 0;
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<ConfigureLessonContentComponent>
  ) {}
  disableTextbox = true;
  ...
  cargarDatos() {
    for (let i in this.dataDialog["images"]) {
      this.images.push({
        value: this.dataDialog["images"][i].idImagen,
        display: this.dataDialog["images"][i].url,
      });
    }
    for (let i in this.dataDialog["types"]) {
      this.types.push({
        value: this.dataDialog["types"][i].idTipo,
        display: this.dataDialog["types"][i].tipo,
      });
    }

...
export class AddCustomModuleComponent implements OnInit {
  ...
  private dialogRef: MatDialogRef<AddCustomModuleComponent>) { 
    for(let i in this.dataDialog['colores']){
      this.colores.push({display:this.dataDialog['colores'][i].color,value:this.dataDialog['colores'][i].idColor});
    }
    for(let i in this.dataDialog['imagenes']){
      this.imagenes.push({display:this.dataDialog['imagenes'][i].url,value:this.dataDialog['imagenes'][i].idImagen});
    }
